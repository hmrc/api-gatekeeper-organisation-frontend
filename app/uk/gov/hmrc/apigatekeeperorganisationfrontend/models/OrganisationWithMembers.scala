...
case class OrganisationWithAllMembersDetailsAndApplications(
    organisation: Organisation,
    collaborators: Set[CollaboratorWithUserDetails],
    applications: List[ApplicationWithCollaborators]
  )

object OrganisationWithAllMembersDetailsAndApplications {

  def apply(organisation: Organisation, members: List[RegisteredOrUnregisteredUser], users: List[User], apps: List[ApplicationWithCollaborators])
      : OrganisationWithAllMembersDetailsAndApplications = {
    val collaborators =
      organisation.collaborators.flatMap(c => CollaboratorWithUserDetails.apply(c, members.find(u => u.userId == c.userId), users.find(u => u.userId == c.userId)))
    OrganisationWithAllMembersDetailsAndApplications(organisation, collaborators, apps)
  }
}
